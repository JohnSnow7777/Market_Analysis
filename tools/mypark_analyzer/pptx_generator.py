...
class PPTXGenerator:
    ...
    def generate(self, data, output_pptx_path):
        ...
        indicators = [
            ...
            ("5) 지역 소비력 및 여가지출", score['scores']['commercial_spending'], 20, "BASA 실측: 골프용품 성장률 1위(+182.4%) 및 스크린골프 상위 20% 월 6,251만원 상권"),
        ]
        ...
        self._add_source_footer(s8, "MYPARK 5-Dimension Diamond Scoring Methodology (22+20+13+15+20=90.0 S-Grade)")
        ...
        # 1. 상단 3대 비용 지표
        s10_metrics = [
            (Inches(0.6), "초기 순투자금 (총 3.86억원)", "3억 8,600만원", "장비 2.0억 + 인테리어 1.56억 + 초도 0.3억"),
            (Inches(4.68), "월 고정비 (인건비+임대료)", f"{fin['monthly_rent']//10000 + 750:,}만원 /월", f"인력 3명(750만) + 임대료({fin['monthly_rent']//10000:,}만)"),
        ...
        kpis = [
            ...
            ("순투자금 회수", f"약 {fin['investment']['payback_months_moderate']:.1f}개월", "(순투자금 3.86억원 기준)", self.c_red)
        ]
