...
class PPTXGenerator:
    ...
    def generate(self, data, output_pptx_path):
        ...
        badges = [
            ...
            (Inches(8.7), "투자금 회수 기간", f"{score['payback_text'].split('(')[0].strip()}", self.c_white)
        ]
        ...
        self._add_header_bar(s3, "사업지 반경 3Km & 자동차 10분 생활권 (", f"약 {demo['total_pop']//10000}만명", ")")
        ...
        for idx, c in enumerate(comps[:4]):
            ...
            hdr_box = s7.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, cur_x, Inches(1.5), card_w, Inches(0.55))
            hdr_box.fill.solid()
            ...
            body_box = s7.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, cur_x, Inches(2.15), card_w, Inches(4.5))
            body_box.fill.solid()
        ...
        self._add_source_footer(s11, "* 산출 근거: 초기 순투자금 3.86억원 기준 / 연 2% 복리 성장률 반영")

        # =========================================================================
        # Slide 12: 종합 결론 및 2대 기대효과 카드 (럭셔리 디자인)
        # =========================================================================
