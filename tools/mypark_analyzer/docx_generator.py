# -*- coding: utf-8 -*-
"""상세 출점 타당성 검토 기획서 DOCX 생성기 (창업주 직접 운영 비교 모델 완비)"""
import os
import docx
...
class DOCXGenerator:
    ...
    def generate(data, output_docx_path):
        ...
        p = doc.add_paragraph()
        p.add_run(f"소상공인 365 및 BASA 빅데이터 분석 결과, 해당 권역 내 스포츠/골프 업종의 월평균 매출액은 약 {comm['monthly_avg_sales']//10000:,}만원 수준으로 안정적인 여가 소비력을 입증하고 있습니다. ")
        ...
        # 4. 재무 및 운영 모델 비교
        h4 = doc.add_heading("4. 재무 타당성 및 운영 방식별 손익/BEP 분석", level=1)
        ...
        p = doc.add_paragraph()
        p.add_run(f"{site['rooms']}타석 운영 기준, 3대 시나리오별 월간 예상 손익은 다음과 같습니다:\\n")
        p.add_run(f"• 보수적 시나리오: 월매출 {c_sc['total_revenue']//10000:,}만원 / 월비용 {c_sc['total_cost']//10000:,}만원 / 월 영업이익 {c_sc['operating_profit']//10000:,}만원\\n")
        p.add_run(f"• 보편적 시나리오: 월매출 {m_sc['total_revenue']//10000:,}만원 / 월비용 {m_sc['total_cost']//10000:,}만원 / 월 영업이익 {m_sc['operating_profit']//10000:,}만원\\n")
        p.add_run(f"• 긍정적 시나리오: 월매출 {o_sc['total_revenue']//10000:,}만원 / 월비용 {o_sc['total_cost']//10000:,}만원 / 월 영업이익 {o_sc['operating_profit']//10000:,}만원\\n\\n")
        
        p.add_run("■ 운영 방식에 따른 손익분기점(BEP) 및 회수 기간 비교:\\n")
        p.add_run(f"1) 오토/위탁 운영 모델 (상주 직원 3명 고용):\\n")
        p.add_run(f"   • 월 고정비 1,640만원, 손익분기점 기기당 1일 0.8회전(월 240명 돌파 시 흑자)\\n")
        ...
        p.add_run(f"   • 손익분기점(BEP) 기기당 1일 단 0.5회전(월 135명 / 일 4.5명 이용 시 돌파)\\n")
        p.add_run(f"   • 보편 가동 시 월 순영업이익 약 {(m_sc['operating_profit']+5000000)//10000:,}만원 (영업이익률 60.0% 달성)\\n")
        p.add_run(f"   • 순투자금 3.36억원 전액 회수 기간: 단 12.8개월 (약 1년 1개월 만에 전액 회수)\\n\\n")
        p.add_run(f"초기 순투자금(CAPEX)은 10타석 120평 기준 총 3억 3,600만원(장비 1.5억 + 인테리어 1.56억 + 초도용품 0.3억)입니다.")
        
        # 5. 결론
        h5 = doc.add_heading("5. 입지 최적성 5대 지표 평가 및 최종 제안", level=1)
        h5.runs[0].font.color.rgb = RGBColor(0, 43, 73)
        p = doc.add_paragraph()
        p.add_run(f"본 사업지는 5대 다이아몬드 스코어링 결과 총점 {score['total_score']}점({score['grade']}등급 - {score['grade_desc']})을 획득하였습니다.\\n\\n")
        p.add_run("■ 가맹점 출점 기대효과:\\n")
        p.add_run(score['value_franchisee'] + "\\n\\n")
        p.add_run("■ 건물주 및 상가 상생 활성화 효과:\\n")
        p.add_run(score['value_landlord'])
