# -*- coding: utf-8 -*-
"""상세 출점 타당성 검토 기획서 DOCX 생성기 (python-docx 기반)"""
import os
import docx
...
class DOCXGenerator:
    ...
    def generate(data, output_docx_path):
        ...
        meta_data = [
            ...
            ("분석 공간 규모", f"전용면적 약 {site['area_pyeong']}평 / {site['rooms']}타석 기준"),
        ...
        p.add_run(f"손익분기점(BEP)은 월 매출 약 {fin['investment']['bep_monthly_sales']//10000:,}만원(타석당 1일 {fin['investment']['bep_turns_per_room']}회전)으로, 보편 가동 기준 순투자금 회수 기간은 약 {fin['investment']['payback_months_moderate']}개월(1년 미만)로 산출되었습니다.")
        
        # 5. 결론
        h5 = doc.add_heading("5. 입지 최적성 5대 지표 평가 및 최종 결론", level=1)
        ...
        p.add_run(f"【 예비 창업자 대상 제안 】\n{score['pitch_franchisee']}\n\n")
        p.add_run(f"【 건물주/임대인 대상 제안 】\n{score['pitch_landlord']}")
