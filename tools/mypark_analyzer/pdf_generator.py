...
class PDFGenerator:
    ...
    def generate(self, data, output_pdf_path=None, charts=None):
        ...
        c.setFont(FONT_REGULAR, 8.5)
        c.setFillColor(self.c_slate)
        # 가맹희망자에게 예상 수익 정보를 제공하는 문서는 가맹사업법의 적용을 받는다.
        # 이 보고서는 '예상매출액 산정서'가 아니라 출점 검토용 참고자료이므로,
        # 그 성격과 한계를 문서 안에서 분명히 밝힌다.
        draw_wrapped_bullets(y_after2 - 30, [
            f"• {_methodology_txt}",
            "• 본 자료는 출점 검토를 돕기 위한 참고자료이며, 가맹사업법상 정보공개서 및 "
            "예상매출액 산정서를 대체하지 않습니다.",
            "• 기재된 매출·손익·회수기간은 표준 모델과 공개 통계에 기반한 추정치로 "
            "장래의 수익을 보장하지 않으며, 실제 실적은 입지·운영·시장 상황에 따라 달라집니다.",
            "• 계약 체결 전 마이파크 사업부서 전문가와 상담하시고, 정보공개서를 반드시 "
            "제공받아 확인하시기 바랍니다.",
        ])
